...
let () =
  ...
  try
    ...
    let p =
      Ttree2istree.program p
      |> Istree2rtl.program
    in
    if debug then Rtltree.print_file std_formatter p;
    if !interp_rtl then begin
        ignore (Rtlinterp.program p);
        exit 0
      end;
    let p = Rtl2ertl.program p in
    if debug then Ertltree.print_file std_formatter p;
    if !interp_ertl then begin
        ignore (Ertlinterp.program p);
        exit 0
      end;
    if debug then Liveness.print_file std_formatter p;
    if debug then Interference.print_file std_formatter p;
    if debug then Coloring.print_file std_formatter p;
    let p = Ertl2ltl.program p in
    if debug then Ltltree.print_file std_formatter p;
    if !interp_ltl then begin
        ignore (Ltlinterp.program p);
        exit 0
      end;
    let code = Ltl2lin.program p in
    let c = open_out (Filename.chop_suffix !ifile ".c" ^ ".s") in
